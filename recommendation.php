<?php
  ...
  require_once(__DIR__ ."/resources/dbconnection.php");
  require_once(__DIR__ . "/resources/email.php");
  ...
  function recommend($user_id){
	  ...
	  $recommends = mysqli_query($connection, $query);
	  if(empty($recommends)){
		  return '';
	  }
  ...
  while ($user = mysqli_fetch_array($users)) {
		  $str_recommends = recommend($user['user_id']);
		  if ($str_recommends!=''){
			  $counter++;
			  $sender->send($user['email_address'],$str_recommends);
		  }
 }
